...
export default function DungeonScreen({ navigation }) {
  ...
  const handleLeave = () => {
    Alert.alert('던전 나가기', '정말 던전을 나가시겠습니까?', [
      { text: '취소', style: 'cancel' },
      { text: '나가기', style: 'destructive', onPress: () => { leaveDungeon(); navigation.goBack(); } }
    ]);
  };

  const handleAttack = () => {
    if (!selectedTarget) return Alert.alert('대상 선택', '공격할 몬스터를 선택해주세요.');
    attack(selectedTarget);
  };

  const handleSkill = (skill) => {
    if (!selectedTarget) return Alert.alert('대상 선택', '스킬을 사용할 몬스터를 선택해주세요.');
    ...
    <View style={styles.container}>
      ...
      <View style={styles.playerStatus}>
        <View style={styles.statusRow}>
          <Text style={styles.playerName}>{player.name} Lv.{player.level}</Text>
      ...
      {dungeonState && (
        <View style={styles.dungeonInfo}>
          <Text style={styles.dungeonName}>{dungeonState.name} - 웨이브 {dungeonState.wave}</Text>
        </View>
      )}
      ...
      <View style={styles.monstersSection}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.monsterScroll}>
          {(dungeonState?.monsters || []).map(monster => {
            const mHpPct = Math.max(0, (monster.hp / monster.maxHp) * 100);
            const isSelected = selectedTarget === monster.id;
            return (
              <TouchableOpacity
                key={monster.id}
                style={[styles.monsterCard, isSelected && styles.monsterCardSelected, monster.isBoss && styles.monsterCardBoss]}
                onPress={() => setSelectedTarget(monster.id)}
              >
                <Text style={styles.monsterEmoji}>{monster.isBoss ? '👑' : '👹'}</Text>
                <Text style={styles.monsterName}>{monster.name}</Text>
                <View style={styles.monsterHpBg}>
                  <View style={[styles.monsterHpFill, { width: `${mHpPct}%` }]} />
                </View>
                <Text style={styles.monsterHpText}>{monster.hp}/{monster.maxHp}</Text>
              </TouchableOpacity>
            );
          })}
          {(!dungeonState?.monsters || dungeonState.monsters.length === 0) && (
            <Text style={styles.noMonsters}>몬스터가 없습니다...</Text>
          )}
        </ScrollView>
      </View>
      ...
      <ScrollView style={styles.combatLog} showsVerticalScrollIndicator={false}>
        ...
        {combatLog.length === 0 && <Text style={styles.logEmpty}>전투 로그가 없습니다.</Text>}
      </ScrollView>

      {/* 액션 버튼 */}
      <View style={styles.actionArea}>
        <TouchableOpacity style={styles.attackBtn} onPress={handleAttack}>
          <Text style={styles.attackBtnText}>⚔️ 기본 공격</Text>
        </TouchableOpacity>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.skillRow}>
          {(player.skills || []).map(skill => (
            <TouchableOpacity key={skill.id} style={styles.skillBtn} onPress={() => handleSkill(skill)}>
              <Text style={styles.skillName}>{skill.name}</Text>
              <Text style={styles.skillMp}>💙 {skill.mpCost}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  playerStatus: { backgroundColor: '#1a0a2e', padding: 14, borderBottomWidth: 1, borderBottomColor: '#2a1a4e' },
  ...
  barBg: { flex: 1, height: 10, backgroundColor: '#0d0618', borderRadius: 5, overflow: 'hidden', marginHorizontal: 8 },
  ...
  monsterScroll: {},
  monsterCard: {
    backgroundColor: '#1a0a2e', borderRadius: 10, padding: 12, marginRight: 10,
    alignItems: 'center', borderWidth: 2, borderColor: '#2a1a3e', minWidth: 90,
  },
  ...
  monsterHpBg: { width: '100%', height: 6, backgroundColor: '#0d0618', borderRadius: 3, marginTop: 6, overflow: 'hidden' },
  ...
  actionArea: { backgroundColor: '#1a0a2e', padding: 12, borderTopWidth: 1, borderTopColor: '#2a1a4e' },
  attackBtn: {
  ...
  skillRow: {},
  skillBtn: {
    backgroundColor: '#2a1a4e', borderRadius: 8, padding: 10, marginRight: 8,
    alignItems: 'center', minWidth: 80, borderWidth: 1, borderColor: '#4a2a7e',
  },
